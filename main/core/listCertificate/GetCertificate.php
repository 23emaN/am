...
Response::json(1, 'Success', [
    ...
    'citizen_id'     => (string) ($row['user_citizen_id'] ?? ''),
    'cpd_no'         => (string) ($row['user_cpd_no'] ?? ''),
    'cpa_no'         => (string) ($row['user_cpa_no'] ?? ''),
    ...
    'num_exam'       => (int) ($row['course_number_exam'] ?? 0),
    'min_score'      => (int) ($row['course_minimum_score'] ?? 0),
    'score'          => $score,
    'percent'        => $percent,
    // สถานะสอบ: จาก tbl_exam_attempt.attempt_pass ครั้งล่าสุด
    'exam_passed'    => ((string) ($row['pass'] ?? '0') === '1') ? 1 : 0,
    // สถานะอนุมัติออกใบรับรอง: จาก enroll_is_completed
